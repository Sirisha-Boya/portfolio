...
export default function ProjectsTab({ selectedProjectTitle, onClearSelectedProject }: ProjectsTabProps) {
  ...
  useEffect(() => {
    if (selectedProjectTitle) {
      const found = PROJECTS.find(p => p.title.toLowerCase() === selectedProjectTitle.toLowerCase());
      if (found) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="bg-white p-6 sketchy-border rotate-[-1deg] paper-texture">
            <h2 className="font-headline-md text-2xl mb-4 text-black flex items-center gap-2">
              <span className="material-symbols-outlined text-cyan-600">bookmark_heart</span>
              My Masterpieces
            </h2>
            <p className="font-body-md text-lg text-gray-600 mb-6">
              Select an item to peel back the tape and view deep development details:
            </p>
            ...
            <div className="flex flex-col gap-3">
              {PROJECTS.map((proj, idx) => {
                ...
                return (
                  ...
                  >
                    <span className="text-2xl filter drop-shadow select-none">{proj.emoji}</span>
                    <div className="flex-1 min-w-0">
                      <div className="font-headline-sm text-lg text-black truncate">{proj.title}</div>
                      <div className="font-body-md text-sm text-gray-600 truncate">{proj.category}</div>
                    </div>
                  </button>
                );
              })}
        ...
        <div className="lg:col-span-8">
          {activeProject && (
            ...
            >
              <div className="washi-tape" style={{ top: '-10px', right: '10%' }}></div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b-2 border-dashed border-neutral-300 pb-6 mb-6">
                <div>
                  <span className="text-4xl filter drop-shadow select-none mb-2 block sm:inline mr-2">{activeProject.emoji}</span>
                  <h1 className="font-headline-md text-3xl text-black inline-block align-middle">{activeProject.title}</h1>
                  <p className="font-body-md text-xl text-cyan-700 italic">{activeProject.category}</p>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeProject.title === "Bank In A Box" && ["Microservices", "ASP.NET Core", "C#", "Docker", "JWT Auth", "D3.js", "Azure"].map((tag, i) => (
                      <span key={i} className="px-3 py-1 border border-black bg-amber-50 rounded-full font-label-code text-sm transform rotate-1 hover:-rotate-1 cursor-default select-none">{tag}</span>
                    ))}
                    {activeProject.title === "CaMS" && ["React.js", "ASP.NET Core", "PostgreSQL", "REST API", "HIPAA Safeguards", "Docker"].map((tag, i) => (
                      <span key={i} className="px-3 py-1 border border-black bg-cyan-50 rounded-full font-label-code text-sm transform -rotate-1 hover:rotate-1 cursor-default select-none">{tag}</span>
                    ))}
                    {activeProject.title === "HaTS" && ["React.js", "C#", "SQL Server", "Kanban Engine", "Cognitive AI Parser"].map((tag, i) => (
                      <span key={i} className="px-3 py-1 border border-black bg-pink-50 rounded-full font-label-code text-sm transform rotate-2 hover:-rotate-2 cursor-default select-none">{tag}</span>
                    ))}
                    {activeProject.title === "XyMACS" && ["React.js", ".NET 6", "Purchase Orders", "CSV Engine", "PDF Generator", "SQL Server"].map((tag, i) => (
                      <span key={i} className="px-3 py-1 border border-black bg-green-50 rounded-full font-label-code text-sm transform -rotate-2 hover:rotate-2 cursor-default select-none">{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 border-b border-dashed border-neutral-300 pb-6 mb-6">
          <div>
            <h2 className="font-headline-md text-2xl text-black flex items-center gap-2">
              <span className="material-symbols-outlined text-yellow-600 animate-pulse">gesture</span>
              Virtual ScribblePad
            </h2>
            <p className="font-body-md text-lg text-gray-600">
              Pick a pencil color and draw or leave a cute hand-drawn greeting directly on our gridded scrapbook!
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2 bg-neutral-50 px-3 py-1.5 border border-black rounded">
              <span className="font-label-code text-sm text-gray-700">Ink:</span>
              <div className="flex gap-1.5">
                {['#000000', '#2563eb', '#dc2626', '#16a34a', '#db2777'].map((col) => (
                  <button
                    key={col}
            ...
            <div className="flex items-center gap-2 bg-neutral-50 px-3 py-1.5 border border-black rounded">
              <span className="font-label-code text-sm text-gray-700">Brush Size:</span>
              <input 
                type="range" 
                min="1" 
                max="12" 
              ...
              />
              <span className="font-label-code text-xs text-black w-4">{brushSize}px</span>
            </div>
